    ...
    <div className="animate-fade-in pb-20">
      ...
      <div className="bg-white p-4 border-b border-pink-100 sticky top-[4.5rem] z-20 shadow-sm mb-6">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={onBack} className="p-2 rounded-full hover:bg-pink-50 text-slate-500 border-2 border-transparent hover:border-pink-100 hover:border-b-4 hover:border-r-4 transition-all active:translate-y-[2px] active:translate-x-[2px] active:border-b-2 active:border-r-2">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex p-1 bg-slate-100 rounded-lg max-w-md mx-auto">
          <button
            ...
            className={`flex-1 py-2 text-xs font-bold rounded-xl border-2 border-b-4 border-r-4 transition-all active:translate-y-[2px] active:translate-x-[2px] active:border-b-2 active:border-r-2 ${viewMode === 'both' ? 'bg-white border-pink-200 border-b-pink-300 border-r-pink-300 text-pink-600' : 'bg-transparent border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            对照模式
          </button>
          <button
            onClick={() => setViewMode('hiragana')}
            className={`flex-1 py-2 text-xs font-bold rounded-xl border-2 border-b-4 border-r-4 transition-all active:translate-y-[2px] active:translate-x-[2px] active:border-b-2 active:border-r-2 ${viewMode === 'hiragana' ? 'bg-white border-pink-200 border-b-pink-300 border-r-pink-300 text-pink-600' : 'bg-transparent border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            平假名 (あ)
          </button>
          <button
            onClick={() => setViewMode('katakana')}
            className={`flex-1 py-2 text-xs font-bold rounded-xl border-2 border-b-4 border-r-4 transition-all active:translate-y-[2px] active:translate-x-[2px] active:border-b-2 active:border-r-2 ${viewMode === 'katakana' ? 'bg-white border-pink-200 border-b-pink-300 border-r-pink-300 text-pink-600' : 'bg-transparent border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            片假名 (ア)
          </button>
        </div>
                  ...
                  {char.h && (
                    <>
                      ...
                      {(viewMode === 'both' || viewMode === 'katakana') && (
                        <div className={`font-medium group-hover:text-pink-400 transition-colors ${viewMode === 'both' ? 'text-sm text-slate-400' : 'text-3xl text-slate-700 font-bold'
                          }`}>
                          {char.k}
                        </div>
                      )}
